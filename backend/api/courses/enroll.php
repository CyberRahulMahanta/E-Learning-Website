...
$data = get_input_data();
$courseInput = $_GET['id'] ?? $data['course_id'] ?? '';
...
if (table_exists($pdo, 'user_course_progress')) {
    ...
    $totalStmt->execute([$resolvedCourseId]);
...
if (table_exists($pdo, 'user_notifications')) {
    ...
    $notifyStmt->execute([
        (int) $authUser['id'],
        json_encode(['courseId' => $resolvedCourseId], JSON_UNESCAPED_SLASHES)
    ]);
}

json_success([
    'alreadyEnrolled' => false
], 'Enrollment successful', 201);
